...
@SofaPrefab
class LegBase(object):
    def __init__(self, parent, mappingInput, name="LegBase", indexInput=0):
        """LegBase is a reusable sofa model of a servo arm for the S90 servo motor

           Parameters:
                parent:        node where the LegBase will be attached
                mappingInput:  the rigid mechanical object that will control the orientation of the servo arm
                indexInput: (int) index of the rigid the LegBase should be mapped to
        """
        ...
        visual = VisualModel(self.node, '../compoz/table_stand.stl', translation=[0., 0., 0.], color=[1., 1., 1., 0.75])
...
class ActuatedArm(object):
    """ActuatedArm is a reusable sofa model of a S90 servo motor and the tripod actuation arm.
           ...
           Node : {
                ...
                LegBase             // The actuation arm connected to ServoMotor.ServoWheel
            }
    """
    ...
    def __init__(self, parent, name="ActuatedArm",
        ...
        LegBase(self.node, self.servomotor.ServoWheel.dofs)
