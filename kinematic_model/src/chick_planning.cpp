...
int main(int argc, char **argv)
{
  ...
  if (!node_handle.getParam("/GPSSuperchickPlugin/type", planner_plugin_name))
    ROS_FATAL_STREAM("Could not find planner: " << planner_plugin_name);
  try
  {
  ...
  req.group_name = "base_bladder";
  moveit_msgs::Constraints pose_goal = kinematic_constraints::constructGoalConstraints("headnball_link", pose, tolerance_pose, tolerance_angle);
  ...
  const robot_state::JointModelGroup* joint_model_group = robot_state.getJointModelGroup("base_bladder");
  robot_state.setJointGroupPositions(joint_model_group, response.trajectory.joint_trajectory.points.back().positions);

  // Now, setup a joint space goal
  robot_state::RobotState goal_state(robot_model);
  std::vector<double> joint_values(1, 0.0);
  joint_values[0] = -2.0;
  // joint_values[3] = -0.2;
  // joint_values[5] = -0.15;
  ...
  moveit_msgs::Constraints pose_goal_2 = kinematic_constraints::constructGoalConstraints("big_bladder_link", pose, tolerance_pose, tolerance_angle);
  /* First, set the state in the planning scene to the final state of the last plan */
  robot_state.setJointGroupPositions(joint_model_group, response.trajectory.joint_trajectory.points.back().positions);
  ...
  req.path_constraints = kinematic_constraints::constructGoalConstraints("big_bladder_link", quaternion);
