    ...
    if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/${directory}" AND IS_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}/${directory}")
        ...
        if(${option})
            ...
            # add_subdirectory(${directory} ${name})
						add_subdirectory(${directory} ${CMAKE_BINARY_DIR})
...
function(iab_generate_package)
    set(oneValueArgs NAME VERSION INCLUDE_ROOT_DIR INCLUDE_INSTALL_DIR INCLUDE_SOURCE_DIR IAB_INSTALL_DIR RELOCATABLE)
    ...
    iab_install_targets("${ARG_NAME}" "${ARG_TARGETS}" "${include_install_dir}" "${ARG_INCLUDE_SOURCE_DIR}" "${ARG_IAB_INSTALL_DIR}" "${ARG_RELOCATABLE}")
...
macro(iab_install_targets package_name the_targets include_install_dir)
    install(TARGETS ${the_targets}
            ...
            BUNDLE DESTINATION "../build" #COMPONENT applications
            )
    ...
    foreach(target ${the_targets})
        ...
        if("${public_header}" STREQUAL "public_header-NOTFOUND" AND NOT "${include_install_dir}" STREQUAL "")
            ...
            if(optional_argv3)
                # ARGV3 is a non-breaking additional argument to handle INCLUDE_SOURCE_DIR (see iab_generate_package)
    ...
    set(iabplugin_install_dir "${CMAKE_BINARY_DIR}")
    ...
    if(optional_argv4)
        ...
        # TODO: add a real argument "iabplugin_install_dir" to this macro
        set(iabplugin_install_dir "${optional_argv4}")
    endif()
    if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/examples")
        install(DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/examples/" DESTINATION "${iabplugin_install_dir}" COMPONENT resources)
    endif()
    if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/scenes")
        install(DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/scenes/" DESTINATION "${iabplugin_install_dir}" COMPONENT resources)
    endif()
